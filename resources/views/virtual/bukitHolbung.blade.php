...
@section('content')
    <section class="w-full max-w-[1440px] mx-auto px-6 sm:px-12 lg:px-28 pt-24">
        ...
        <p class="mt-3 dark:text-white/80 font-sans max-w-3xl">Panorama Danau Toba dari perbukitan savana yang memukau.</p>
    </section>
    ...
    <section id="detail-section" class="w-full max-w-[1440px] mx-auto px-6 sm:px-12 lg:px-28 py-16">
        ...
        <div class="mt-12 grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            ...
            <div>
                ...
                <p class="mt-4 dark:text-white/80 font-sans">Bukit Holbung adalah salah satu destinasi wisata alam yang terletak di Kabupaten Samosir, Sumatera Utara. Bukit ini dikenal karena pemandangannya yang memukau, dengan latar belakang Danau Toba yang luas dan indah. Bukit Holbung menjadi favorit para wisatawan yang mencari tempat untuk menikmati keindahan alam dan berfoto dengan latar belakang yang menakjubkan. Dari puncak bukit, pengunjung dapat melihat panorama Danau Toba yang dikelilingi oleh pegunungan hijau.</p>
                <p class="mt-4 dark:text-white/80 font-sans">Untuk mencapai puncak Bukit Holbung, jalur pendakiannya cukup landai dan ramah bagi pemula. Suasana tenang, angin sepoi, dan padang rumput yang luas menghadirkan pengalaman yang mengesankan, terutama saat matahari terbit atau terbenam.</p>
            </div>
        </div>

        <div class="mt-10 bg-white/5 rounded-2xl p-6">
            <span class="text-red-400 font-semibold">Referensi</span>
            <p class="mt-2 dark:text-white/80 font-sans">Aldira. (2022, Juni 25). Pesona Danau Toba dari Bukit Holbung. Lemon8. <a class="text-red-400 underline" href="https://www.lemon8-app.com/aldira/7112998082899411457?region=id" target="_blank" rel="noopener">tautan</a>.</p>
        </div>
        ...
        <nav class="mt-12 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('virtual.funland') }}" class="inline-flex items-center gap-2 bg-black/10 hover:bg-black/20 dark:bg-white/10 dark:hover:bg-white/20 dark:text-white/90 dark:hover:text-white px-4 py-2 rounded-lg transition-colors">← Prev: Mikie Funland</a>
                <a href="{{ route('virtual.air-terjun-piso') }}" class="inline-flex items-center gap-2 bg-black/10 hover:bg-black/20 dark:bg-white/10 dark:hover:bg-white/20 dark:text-white/90 dark:hover:text-white px-4 py-2 rounded-lg transition-colors">Next: Air Terjun Sipiso-piso →</a>
            </div>
        </nav>
    </section>
    ...
    <h2 class="text-2xl text-bold dark:text-white"> Virtual Tour Lainnya </h2>
        <div class="mt-8 flex flex-wrap gap-3">
            <a href="{{ route('virtual.bukit-holbung') }}" class=" bg-black/20 dark:bg-white/20 hover:bg-black/20 dark:hover:bg-white/20 dark:text-white/90 hover:text-black/80 dark:hover:text-white px-4 py-2 rounded-lg transition-colors">Bukit Holbung</a>
