            ...
            @foreach ($lessons['lesson'] as $index => $lesson)
                ...
                <div>
                    <button id="controlsAccordionItem{{ $index }}" type="button"
                        class="flex w-full items-center justify-between gap-4 p-4 text-left text-text-primary rounded-lg bg-bg-card bg-opacity-10 hover:bg-bg-card hover:bg-opacity-5 transition-all duration-100 ease-in-out border border-white/20"
                        aria-controls="accordionItem{{ $index }}"
                        x-on:click="selectedAccordionItem = selectedAccordionItem === {{ $index }} ? null : {{ $index }}"
                        x-bind:class="selectedAccordionItem === {{ $index }} ? 'text-white font-bold bg-gray-700' :
                                    ...
                                    <div class="bg-bg-card bg-opacity-20 hover:bg-opacity-10 p-4 flex justify-between mb-1 rounded-md cursor-pointer transition-all duration-150"
                                        ...
                                        <div class="flex items-center my-auto gap-4 cursor-pointer">
                                            ...
                                            <svg xmlns="http://www.w3.org/2000/svg" class="size-5" viewBox="0 0 512 512"
                                                ...
                                                stroke="{{ $isCompleted ? 'none' : 'white' }}" stroke-width="10">
                                                <circle cx="256" cy="256" r="200" />
                                            </svg>

                                            <p>{{ $lesson_part->part_name ?? 'Lesson Part' }}</p>
                                        </div>
                                        <svg xmlns="http://www.w3.org/2000/svg" class="size-5 fill-white"
                                            viewBox="0 0 640 640">
                                            <path
